			...
			<div class="general_wrapper">
				...
				<c:forEach var='reserve' items='${reserveList}'>
					...
					<tr class='reserveList_row1'>
						...
						<td class='reserveList_d3'>2021.10.26(목)<c:out value='${reserveList.e_startDate}'/>~<c:out value='${event.e_endDate}'/>2021.10.28(목)<br>
						</td>
						<td class='reserveList_d4'><c:out value='${reserveList.eh_location}'/>kosta 가산 </td>
						...
						<td class='reserveList_d7'><c:out value='${reserveList.ticket_payment_status}'/>결제완료</td>
					</tr>
					<tr class='reserveList_row2'>
						<td class='reserveList_d2'>
							<c:out value="${reserveList.account_name}"/><br>
							<c:out value="${reserveList.account_phoneNumber}"/><br>
							<c:out value="${reserveList.account_id}"/>@<c:out value="${reserveList.account_email}"/>uuufg3322@naver.com
						</td>
						<td class='reserveList_d3'></td>
						<td class='reserveList_d4'></td>
						<td class='reserveList_d5'></td>
						<td class='reserveList_d6'><c:out value="${reserveList.ticket_reservation_amount}"/> 매</td>
						<td class='reserveList_d7'></td>
					</tr>
					<!--더미2 시작-->
					<tr class='reserveList_row2'>
						<td class='reserveList_d1'rowspan='2'>
							<c:out value="${reserveList.ticket_reservation_time}"/>2021.10.24
							(12:42:30)
							<div>진행중</div>
						</td>
						<td class='reserveList_d2'>
							<b><c:out value="${reserveList.e_name}"/>코스타 223기 인성교육 "화성 입국 면접"</b>
							<div class="img_reserveList"><img src='${pageContext.request.contextPath}/resources/images/1ron.jpg' alt='reserveList_image'></div>
						</td>
						...
						<td class='reserveList_d4'><c:out value="${reserveList.eh_location}"/>kosta 가산 </td>
						<td class='reserveList_d5'>최종금액 <c:out value="${reserveList.ticket_payment_price}"/> 원</td>
						<td class='reserveList_d6'>
							<button class='cancel_openMask' value="예약취소">예약취소</button><br>
							<button class='ticket_openMask' value="상세보기">상세보기</button>
						</td>
					...
					<tr class='reserveList_row2'>
						<td class='reserveList_d2'>
						이정준<br>
						<c:out value="${reserveList.account_phoneNumber}"/>01046666218<br>
						<c:out value="${reserveList.account_id}"/>@<c:out value="${reserveList.account_email}"/>uuufg3322@naver.com
						</td>
						<td class='reserveList_d3'></td>
						<td class='reserveList_d4'></td>
...
<form action='' class='cancel_form_modal'>
	...
	<h3>신청내역</h3>
		<div>
			신청자 정보<br>
			이름 : <c:out value="${reserveList.account_name}"/>
		</div>
		<div>
			이메일
			<hr>
		...
		<div>
			전화번호 : <c:out value="${reserveList.account_phoneNumber}"/><br>
			수량 : <c:out value="${reserveList.ticket_reservation_amount}"/>매<br>
			금액 :  <c:out value="${reserveList.ticket_reservation_amount}"/>원<br>
			결제금액 :  <c:out value="${reserveList.ticket_payment_price}"/>원
		</div>
	</div>
...
<%-- 					 src='${pageContext.request.contextPath}/resources/images/steve2.jpg' alt='exh_image'> --%>
		<c:out value="${reserveList.e_name}"/>
		</div>
		<div class='ticket_content'>
		</div>
	<div class='ticket_footer'> 

		매수 :<c:out value="${reserveList.ticket_reservation_amount}"/>매 
		신청기간 : <c:out value="${reserveList.e_name}"/><br> 
